...
class Title {
  mesh: any;
  constructor({ gl, plane, text, textColor = "#E8E0D4", font = "bold 28px 'Cormorant Garamond'" }: any) {
...
class MediaItem {
  ...
  constructor({ geometry, gl, image, index, length, scene, screen, viewport, text, bend, textColor, borderRadius = 0, font }: any) {
    ...
    // Create title
    this.title = new Title({ gl: this.gl, plane: this.plane, text, textColor, font });
...
class GalleryApp {
  ...
  constructor(container: HTMLElement, { items, bend, textColor = "#E8E0D4", borderRadius = 0.05, font = "bold 28px 'Cormorant Garamond'", scrollSpeed = 2, scrollEase = 0.05 }: any = {}) {
  ...
  addEventListeners() {
    ...
    // Only resize on window; all interaction events scoped to container
